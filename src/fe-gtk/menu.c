...
menu_shell_title (ZvtTerm * term, int type, char *newtitle, session * sess)
{
	...
	case VTTITLE_WINDOWICON:
		gtk_entry_set_text (GTK_ENTRY (sess->gui->topic), newtitle);
	}
}
...
menu_newshell_set_palette (session *sess)
{
	...
	zvt_term_set_color_scheme ((ZvtTerm *) sess->gui->xtext,
...
menu_newshell_tab (GtkWidget * wid, struct session *sess)
{
	...
	if (!name)
		...
		name++;
	safe_strcpy (sess->channel, name, CHANLEN);
	fe_set_channel (sess);

	gtk_widget_destroy (sess->gui->xtext);
	gtk_widget_destroy (sess->gui->vscrollbar);

	sess->gui->xtext = zvt_term_new ();
	zvt = (ZvtTerm *) sess->gui->xtext;
	...
	GTK_WIDGET_UNSET_FLAGS (sess->gui->topic, GTK_CAN_FOCUS);
	...
	g_signal_connect (G_OBJECT (sess->gui->xtext), "child_died",
							G_CALLBACK (shell_exit), sess);
	g_signal_connect (G_OBJECT (sess->gui->xtext), "title_changed",
							G_CALLBACK (menu_shell_title), sess);
	gtk_widget_destroy (sess->gui->inputgad->parent);

	sess->gui->inputgad = 0;
	sess->gui->toolbox = 0;
	gtk_widget_grab_focus (sess->gui->xtext);

	switch (zvt_term_forkpty ((ZvtTerm *) sess->gui->xtext, 0))
	{
	case -1:
		break;
...
#ifdef USE_XLIB
		sprintf (buf, "%d",
					(int) GDK_WINDOW_XWINDOW (GTK_WIDGET (sess->gui->xtext)->
													  window));
#endif
