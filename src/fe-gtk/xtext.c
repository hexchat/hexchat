...
gtk_xtext_adjustment_set (xtext_buffer *buf, int fire_signal)
{
	...
	if (buf->xtext->buffer == buf)
	{
		...
		if (gtk_adjustment_get_upper (adj) == 0)
			gtk_adjustment_set_upper (adj, 0);

		gtk_widget_get_allocation (GTK_WIDGET (buf->xtext), &alloc);

		gtk_adjustment_set_page_size (adj, alloc.height / buf->xtext->fontsize);
...
gtk_xtext_scrolldown_timeout (GtkXText * xtext)
{
	...
	if (buf->last_ent_end == NULL ||	/* If context has changed OR */
		 ...
		 gtk_adjustment_get_value (adj) >= gtk_adjustment_get_upper (adj) - gtk_adjustment_get_page_size (adj))
	{
